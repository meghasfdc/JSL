...
abstract public class IterativeProcess<T> implements ObservableIfc, IterativeProcessIfc {
     ...
     */
    public static final int NO_STEPS_EXECUTED = JSL.getNextEnumConstant();

    /**
     * Indicates whether the iterative process has completed running all steps
     */
    public static final int COMPLETED_ALL_STEPS = JSL.getNextEnumConstant();

    /**
     * Indicates whether the iterative process has exceeded its maximum
     * execution time
     */
    public static final int EXCEEDED_EXECUTION_TIME = JSL.getNextEnumConstant();

    /**
     * Indicates whether the iterative process ended due to the being stopped
     */
    public static final int MET_STOPPING_CONDITION = JSL.getNextEnumConstant();

    /**
     * Indicates that the iterative process is in progress
     */
    public static final int UNFINISHED = JSL.getNextEnumConstant();
    ...
    public void turnOnLogReport(String name) {
        ...
        String s;
        if (name != null) {
            s = name + " " + getName() + " LogReport";
        } else {
            s = getName() + " LogReport";
        }
        myIPLogReport = new IPLogReport(s);
