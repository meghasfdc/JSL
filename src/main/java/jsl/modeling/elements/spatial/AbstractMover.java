...
abstract public class AbstractMover extends SpatialModelElement implements MoverIfc, VelocityIfc {
     ...
     */
    public static final int TRIP_STARTED = JSL.getNextEnumConstant();

    /**
     * Indicates that a AbstractMover ended a trip
     */
    public static final int TRIP_ENDED = JSL.getNextEnumConstant();

    /**
     * Indicates that a AbstractMover had its trip canceled
     */
    public static final int TRIP_CANCELED = JSL.getNextEnumConstant();

    /**
     * Indicates that a AbstractMover started a movement
     */
    public static final int MOVE_STARTED = JSL.getNextEnumConstant();

    /**
     * Indicates that a AbstractMover ended a movement
     */
    public static final int MOVE_ENDED = JSL.getNextEnumConstant();
     ...
     */
    private JSLEvent myCurrentMovementEvent;

    /**
     * ActionListener that handles the end of movement event
     */
    private EndMovementListener myEndMovementListener;
    ...
    public AbstractMover(ModelElement parent, String name, SpatialModel spatialModel, CoordinateIfc coordinate) {
        ...
        myEndMovementListener = new EndMovementListener();
    }
    ...
    private void startMovement_() {
        ...
        if (checkForCollision()) {
            ...
            myCurrentMovementEvent = scheduleEvent(myEndMovementListener,
                    myMovementTime, "EndMovement");
            myMovingFlag = true;
    ...
    private class EndMovementListener implements EventActionIfc {
