...
public class Executive implements IdentityIfc, ObservableIfc, IterativeProcessIfc {
     ...
     *
     */
    public final static int AFTER_EXECUTION = JSL.getNextEnumConstant();

    /**
     * Used when observers are notified of initialization
     *
     */
    public final static int INITIALIZED = JSL.getNextEnumConstant();

    /**
     * Used to indicate to observers that an event will be executed
     *
     */
    public final static int BEFORE_EVENT = JSL.getNextEnumConstant();

    /**
     * Used to indicate to observers that an event will be executed
     *
     */
    public final static int AFTER_EVENT = JSL.getNextEnumConstant();

    /**
     * Indicates that the executive stopped
     *
     */
//    public final static int STOPPED = JSL.getNextEnumConstant();
    /**
    ...
    @Override
    public void turnOnLogReport(String name) {
        myEventExecutionProcess.turnOnLogReport(name);
    }

    @Override
    public final void turnOnLogReport() {
        myEventExecutionProcess.turnOnLogReport();
    }
    ...
    public final <T> JSLEvent<T> scheduleEvent(EventActionIfc listener,
            double time, String name, int priority, T message, ModelElement theElementScheduling) {
    ...
    public final void turnOnDefaultEventTraceReport(String name) {
        ...
        if (myTraceReportFlag == false) {
            ...
            myTraceReport = new ExecutiveTraceReport(s);
...
// the only side-effect is in the Executive test suite
        ...
        myEndEvent = scheduleEvent(new EndEventActionListener(),
                time, "End Replication",
                JSLEvent.DEFAULT_END_REPLICATION_EVENT_PRIORITY, null, theElement);
        return myEndEvent;
    }
    ...
    private class EndEventActionListener implements EventActionIfc {
