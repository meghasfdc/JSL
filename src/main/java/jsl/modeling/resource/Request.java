 ...
 * allocate
 * for a period of time. A builder pattern is available to facilitate
 * construction of requests.
 * <p>
 * The request of the resourceUnit may be preempted or
 * not. The preemption rule determines how the request is handled if the
 * resourceUnit
 * attempts a preemption because of a failure or inactive period.
 * <p>
 * The rules are: NONE = do not allow preemption, the request cannot be
 * preempted
 * <p>
 * RESUMABLE = request resumes with the time remaining after preemption
 * <p>
 * RESTART = the request restarts using its original time after preemption<p>
 * CANCEL = the request should be cancel upon preemption
 * <p>
 * <p>
 ...
 * <p>
 * RejectedState = rejected after creation, no further transitions<p>
 * CanceledState = can cancel from ReadyState or AllocatedState, no further
 * transitions
 * <p>
 * AllocatedState = using the resourceUnit, may preempted, canceled, or finished
 * <p>
 * Preempted = preempted from using the resourceUnit, can resumed or cancel
 * <p>
 * Finished = finished its life-cycle, no further transitions.
 * <p>
 * @author rossetti
 */
public class Request extends QObject {
    ...
    private final Finished myFinishedState = new Finished();
    ...
    private final ReadyState myReadyState = new ReadyState();
...
//        }
        ...
        public BuildStep entity(Object entity) {
            if (entity == null) {
                throw new IllegalArgumentException("The attached entity was null.");
            }
            this.entity = entity;
            return this;
        }
    ...
    public final boolean isFinished() {
        return myState == myFinishedState;
    }
    ...
    public final boolean isPreviousStateReady() {
        return myPrevState == myReadyState;
    }
    ...
    public final double getTimeCompleted() {
        return myFinishedState.getTimeStateEntered();
    }
    ...
    void reject(double time) {
        myState.rejected(time);
    }

    /**
     * Can only enterWaitingState from the ReadyState state
     */
    void enterWaitingState(Queue queue, double time) {
        myState.queued(queue, time);
    }

    /**
     * Can only exitWaitingState() from the WaitingState
     */
    void exitWaitingState(Queue queue, double time) {
        myState.dequeued(queue, time);
    }
    ...
    /**
     * A Request that is ready can wait, be canceled, or transition
     * to being in allocate.
     */
    protected class ReadyState extends RequestState {
    ...
    protected class Finished extends RequestState {
